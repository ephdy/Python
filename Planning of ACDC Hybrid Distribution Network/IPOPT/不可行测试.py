...
def PYOMO_Solve(S, Edges, Gain_DG, Default=None):
    ...
    # ========= 变量 =========
    model.V = Var(model.AC, model.t, bounds=(0.9, 1.1))
    model.theta = Var(model.AC, model.t, bounds=(-3.14, 3.14))
    ...
    # sum( sum(m.Pd[i,t]-m.P_load[i,t] for i in m.N) for t in m.t )/sum(m.Pd[i,t] for i in m.N for t in m.t)+1e-6 * sum(m.P_load[i,t]**2 for i in m.N for t in m.t)
    ...
    if not check_optimal_termination(result):
        ...
        log_infeasible_constraints(model)  # 打印不可行约束
