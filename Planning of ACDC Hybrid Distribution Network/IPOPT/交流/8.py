...
def PYOMO_Solve(Edges, Gain_DG, Default=None):
    ...
    model.P = Var(model.N, model.N, model.t, bounds=(-0.25, 0.25))
    model.Q = Var(model.N, model.N, model.t, bounds=(-0.25, 0.25))
    ...
    def line_AC_limit_rule(m, x, y, t):
        if (x, y) in model.E:
            ...
            return Constraint.Skip
    # # 在模型中添加约束
    model.line_P = Constraint(model.N,model.N, model.t, rule=line_power_rule)
    model.line_Q = Constraint(model.N,model.N, model.t, rule=line_reactive_rule)
    model.line_AC_limit = Constraint(model.N,model.N, model.t, rule=line_AC_limit_rule)
...
def fun3(path):
    ...
    X = data1.iloc[:, :33].values
    M = data1.iloc[:, 33].values
    E = data1.iloc[:, 34].values
    print(X[0],M[0],E[0])
...
if __name__ == '__main__':
    ...
    fun3('./snap/交流样本_42.csv')
    fun3('./snap/交流样本_65.csv')
