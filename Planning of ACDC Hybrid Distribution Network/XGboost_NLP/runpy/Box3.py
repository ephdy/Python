...
def GAMS_Solve(S,Edges,Gain_DG,Default=None):
    ...
    vsc_loss[VSC, t] = (
            P_vsc_loss[VSC, t]**2 == 1e-4 *( P_vsc_ac[VSC, t]**2 + Q_vsc[VSC, t]**2)
    )
    ...
    line_vsc[VSC, t] = (P_vsc_ac[VSC, t]**2+Q_vsc[VSC, t]**2 <= 0.25 * 0.25 * 0.8 * 0.8)

    line_DC = gp.Equation(m, "line_DC", domain=[k, l, t])
    line_DC[k, l, t] = (
            (Vdc[k, t]*(Vdc[k, t] - Vdc[l, t]) * R1[k, l])**2 <= 0.25 * 0.25 * 0.8 * 0.8
    )
