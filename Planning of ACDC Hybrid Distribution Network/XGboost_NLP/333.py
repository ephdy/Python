...
def GAMS_Init():
    ws = GamsWorkspace()
    job = ws.add_job_from_file("opf.gms")
...
def GAMS_Solve(S,Edges,Gain_DG,Default=None):
    ...
    Pmax_DG = gp.Parameter(m, "Pmax_DG", domain=[N, t])
    ...
    vsc_loss[VSC, t] = (
            P_vsc_loss[VSC, t] == 2e-2 *( P_vsc_ac[VSC, t]**2 + Q_vsc[VSC, t]**2)**0.5
    )
    ...
    opf.freeze(modifiables=[Pmax_DG])
    eeee=[1,1,1]
    # for i in range(3):
    #     Pmax_DG.setRecords(get_DG(eeee[i])[0])  # 更新参数
    #     print(Pmax_DG.records)
    #     print(P_DG.records)
    #     opf.solve(solver="conopt")
    #
    #     print(f"Model Status: {opf.model_status}")
    #     print(f"Solver Status: {opf.solver_status}")
    #     print(f"b={eeee[i]}, obj={opf.objective_value}")

    Pmax_DG.setRecords(get_DG(1)[0])  # 更新参数
    print(Pmax_DG.records)
    print(P_DG.records)
    opf.solve(solver="CONOPT")
    ...
    print(f"b=, obj={opf.objective_value}")
    # opf.unfreeze()  # 释放资源
    ...
    # print(d2)
    # print(e)
    # return opf.objective_value,result.loc[0, "Model Status"]
